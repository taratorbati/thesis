...
The control action at day $k$, denoted as $\mathbf{U}_k$, dictates the daily irrigation depth applied to the field. The action space $\mathcal{U}$ is bounded by the physical realities of the pumping and distribution infrastructure. 

Let $u^n_k$ represent the irrigation command dispatched to agent $n$ on day $k$. The controller is strictly constrained by a maximum daily actuator limit, $u_{max}$, establishing a bounded continuous action space:
...
where $\mathbf{W}_k$ represents the stochastic meteorological disturbance at day $k$ (e.g., rainfall, evapotranspiration, and solar radiation). 

The overarching objective for any deployed controller is to find a policy $\pi(\mathbf{X}_k) \to \mathbf{U}_k$ that minimizes an expected cumulative cost function $J$ over the finite horizon $K$:
...
The MPC optimization problem is formulated to maximize crop yield while minimizing resource expenditure and environmental stress over a prediction horizon $H_p$. To ensure stable convergence within the interior-point solver (IPOPT), the cost function $J$ is heavily normalized. Each term is scaled to $\mathcal{O}(1)$ using agronomically and economically derived reference values.

The total objective function is defined as a sum of five normalized terms:
...
Due to the topographical cascade routing, water accumulates in the lower-elevation sink agents ($n \in \mathcal{S}$). To prevent crop asphyxiation and excessive runoff pooling, a penalty is applied to surface ponding ($x_5$):

\begin{equation}
    J_{ponding} = \alpha_4 \sum_{j=0}^{H_p-1} \frac{1}{|\mathcal{S}|} \sum_{n \in \mathcal{S}} \frac{x_5^n(k+j)}{x_{5,ref}}
\end{equation}

where $x_{5,ref} = 10.0$ mm is the acceptable standing water limit, and $\alpha_4 = 0.5$ places a high priority on preventing localized flooding in the topological sinks.
...
\begin{equation}
    ...
    \end{cases}
\end{equation}
This runoff is immediately distributed as $\frac{\phi_2^{(n)}}{N_r^{(n)}}$ to the $W_{surf}$ of its downstream neighbors within the same discrete time step. Terminal sink agents are defined by $N_r = 0$, enforcing $\phi_2 = 0$ and allowing the solver to accurately predict localized ponding ($x_5$).
...
A naive multi-agent implementation---where a single monolithic policy network directly outputs 130 independent actions from the 660-dimensional state---would fail to exploit the spatial symmetry of the field and require an intractable number of parameters to train effectively. 

To overcome this, the architecture employs Centralized Training with Decentralized Execution (CTDE).
