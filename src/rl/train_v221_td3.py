...
# -----------------------------------------------------------------------------
# v2.21 = v2.20's exact-n-step trainer + GAMMA-CORRECT potential-based biomass
# shaping (r1 = (gamma*x4_t - x4_{t-1})/X4_REF). When biomass_shaping=True the
# trainer sets the env's biomass_shaping_gamma = gamma_base, so the discounted
...
from src.rl.configs_v221 import CONFIGS, GAMMA_BASE
...
def train_td3_v221(
    ...
    run_name = f"td3_v221_{cfg['label']}_seed{seed}"
    ...
    print(f"\n{'='*72}")
    print(f"  TD3 training - v2.21 (gamma-correct biomass shaping) - config {config_name} ({cfg['label']}) - seed {seed}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Train TD3 v2.21: v2.20 exact-n-step base + gamma-correct potential-based "
