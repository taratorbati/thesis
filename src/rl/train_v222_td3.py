...
# -----------------------------------------------------------------------------
# v2.22 = v2.21c (additive terminal-yield) + Markov-r5: when expose_prev_u=True the
# trainer uses IrrigationEnvPrevU (u_{t-1} as a 9th per-agent obs feature) and the
# 9-feature actor+critic (networks_td3_prevu), so the delta-u reward r5 is finally
...
from src.rl.gym_env_prev_u import IrrigationEnvPrevU
...
from src.rl.configs_v222 import CONFIGS, GAMMA_BASE
...
def train_td3_v222(
    ...
    # v2.22: Markov-r5. When expose_prev_u=True, use the prev_u env (u_{t-1} as a
    # 9th per-agent feature -> 1227-dim obs) AND the matching 9-feature actor+critic
    # (networks_td3_prevu). Otherwise the v2.21c 8-feature path (1097-dim). The
    # 9-feature network is byte-identical to v2.21c's except the input width.
    if expose_prev_u:
        EnvCls    = IrrigationEnvPrevU
        PolicyCls = TD3VDNPolicyPrevU
    else:
        EnvCls    = IrrigationEnv
        PolicyCls = TD3VDNPolicy

    run_name = f"td3_v222_{cfg['label']}_seed{seed}"
    ...
    config = {
        ...
        "policy_class": PolicyCls.__name__,
    ...
    print(f"\n{'='*72}")
    print(f"  TD3 training - v2.22 (Markov-r5: prev_u + 9-feature net) - config {config_name} ({cfg['label']}) - seed {seed}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Train TD3 v2.22: v2.21c + prev_u (u_{t-1} in obs) so r5 is Markov, with the "
