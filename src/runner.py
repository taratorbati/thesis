...
#
# v2.2: _compute_final_metrics updated to report field-wide ponding stats
# instead of sink-only, consistent with terrain.py v2.0 (no true sinks).
# =============================================================================
...
def _compute_final_metrics(trajectory, crop, terrain, budget_total):
    ...
    v2.2: Reports field-wide ponding metrics instead of sink-only metrics,
    consistent with fractional routing boundary conditions (terrain.py v2.0).
    """
    HI = crop.get('HI', 0.45)
    fc_total = crop['theta6'] * crop['theta5']
    p = crop.get('p', 0.20)
    ...
    yield_kg_ha           = terminal_biomass_g_m2 * HI * 10.0
    water_used_mm         = float(u.sum() / N)
    ...
    drought_days_per_agent  = float((x1 < stress_threshold).sum() / N)
    waterlog_days_per_agent = float((x1 > fc_total).sum() / N)

    # Field-wide ponding metrics (replaces sink-only metrics from v1)
    pond_days_per_agent = float((x5 > 5.0).sum() / N)
    x5_max_mm = float(x5.max())
    x5_mean_mm = float(x5.mean())
    ...
    return {
        ...
        'pond_days_per_agent':     pond_days_per_agent,
        'x5_max_mm':              x5_max_mm,
        'x5_mean_mm':             x5_mean_mm,
